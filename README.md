...
當你已經設定好 ChatGPT 5.6 Luna Max Fast，卻在某次對話中臨時想切換到 Sol
High／Sol xHigh 處理時，你需要的就是 `codex-sol-the-master`。它提供一套清楚、
可稽核的雙層委派方案：透過精確的繁中或英文暗號，把單一完整任務交給對應的 Sol
Agent；任務完成、失敗或中止後，控制權回到原本主代理，不會把整個對話永久切換。
同時附上安全安裝範例、正反向測試與回到預設設定的驗證流程，讓你能在保留原本主
代理設定的前提下，按需要召請更深的推理能力。

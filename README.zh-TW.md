...
指令只作用於目前這一項任務，不會永久改變主 Agent 的模型、推理強度或
速度設定。辨識必須採用逐字精確比對；不得使用模糊比對、子字串比對、標點
正規化或語意相似度。兩個指令不可互相誤觸發。

## 專案內容

- `SOL_DELEGATION_SETUP_REQUEST.md`：保留原始設定需求。
